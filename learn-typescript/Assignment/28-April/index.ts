...
function filterByLength(array: string[], n: number) {
...
console.log(
  filterByLength(
    ["apple", "banana", "grapes", "apricot", "guava", "avocado"],
    5
  )
);
...
function countOccurrences(array: any[], element: any) {
...
function convertFahrenheitToCelsius(fahrenheitTemperatures: number[]) {
...
function countCharacter(string: string, character: string) {
  let count = 0;
...
// write a function to log only the tasks that are not yet completed
function logNotCompletedTasks(toDoList: any[]) {
...
function logArrayInReverse(array: any[]) {
  for (let i = array.length - 1; i >= 0; i--) {
...
function calculate(operand1: number, operand2: number, operator: string) {
  let result = 0; // initialize result to 0
  switch (
    operator // switch case to perform the operation based on the operator
  ) {
    case "+":
